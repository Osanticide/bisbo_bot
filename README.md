...
O PostgreSQL armazena os dados persistentes dos perfis. As credenciais e
configurações locais são mantidas no arquivo `.env`, que não deve ser
enviado ao GitHub.

## Organização do projeto

O código é organizado em módulos. Os comandos ficam em `app/cogs/`,
enquanto a conexão e as operações relacionadas ao banco ficam em
`app/database/`.
...
**Planejar → Implementar → Testar → Revisar → Versionar → Integrar**

O repositório GitHub é a referência central do código. O bot e o banco
PostgreSQL de produção são executados na VPS.

## Próximas etapas

-   Planejar e implementar o sistema de XP e níveis.
-   Integrar a concessão de XP por mensagens válidas.
-   Desenvolver os sistemas de economia e empregos.
-   Expandir a documentação conforme novos comandos e recursos forem
    implementados.
